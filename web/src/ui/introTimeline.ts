...
const LORE_BEATS: LoreBeat[] = [
  ...
  },
  {
    startMs: 11600,
    endMs: 15800,
    line: 'Every step now moves all bodies at once.',
  },
];
...
export function sampleIntroVisualState(elapsedMs: number, width: number, height: number): IntroVisualState {
  const clamped = clampElapsed(elapsedMs);
  const phase = introPhaseAt(clamped);
  const progress = clamp01(clamped / INTRO_DURATION_MS);
  const lockstepAmount = clamp01((clamped - 6500) / 2900);
  const fractureFlash = pulseCenter(clamped, 6200, 620);
  const corePulse = 0.5 + 0.5 * Math.sin(clamped / 165);
  const gridDrift = (clamped * 0.028) % 36;

  let activeLine = '';
  let activeLineAlpha = 0;
  for (const beat of LORE_BEATS) {
    const alpha = segmentAlpha(clamped, beat.startMs, beat.endMs);
  ...
  const path = lockstepPath(clamped - 9800);
  ...
  const explorers: IntroExplorer[] = EXPLORER_ANCHORS.map((anchor, index) => {
    const independentX =
      centerX +
      anchor.x * width +
      Math.sin(clamped * 0.00115 * (index + 1) + index * 1.7) * width * 0.09;
    const independentY =
      centerY +
      anchor.y * height +
      Math.cos(clamped * 0.00103 * (index + 2) + index * 2.2) * height * 0.085;
    ...
    const size = baseSize * (0.9 + 0.18 * (0.5 + 0.5 * Math.sin(clamped / 210 + index * 0.9)));
